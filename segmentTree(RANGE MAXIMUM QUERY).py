...
class SegmentTree       :
    ...
    def constructSegmentTree(self,arr,i,s,e):
        if (s == e - 1):
            ...
            self.r[i] = max(self.constructSegmentTree(arr,2*i+1,s,m),self.constructSegmentTree(arr,2*i+2,m,e))
        return self.r[i]
    ...
    def rangeQueryMain(self,i,s,e,rs,re):
        ...
        if rs <= s+1 and e <= re:
            return self.r[i]
        m = self.findMiddle(s,e)
        return  max(self.rangeQueryMain(2*i+1,s,m,rs,re),self.rangeQueryMain(2*i+2,m,e,rs,re))
